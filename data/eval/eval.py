...
def extract_eval_data(inp_file, out_file, devset=None, evalset=None, corpus=None, max_sents=250, annotated=True):
  ...
  with open("/content/drive/My Drive/Colab Notebooks/genre/eval/jan05_2024/final_dataset.csv", "r", encoding="utf8") as existds:
    reader = csv.reader(existds, delimiter=",")
    headers = next(reader)
    existds_data = list(reader)
    for row in existds_data:
      existdata.append(row[3])
  ...
  with open(inp_file, "r", encoding="utf8") as inp:
    ...
    for row in data:
      ...
      if clean_tags(row[NON_INCL_SENT_ROW]) not in dev_data and clean_tags(row[NON_INCL_SENT_ROW]) not in edata and clean_tags(row[NON_INCL_SENT_ROW]) not in existdata:
        print(row)
        if count >= max_sents:
          break
        ...
        try:
          if not annotated:
            ...
            incl_sents.append(test_sentence(row[1], tests=True))
          else:
            inflection_data = []
            ...
            non_incl_sent_cleaned = clean_tags(non_incl_sent)

            member_phrases = [get_collective_noun_data(member_noun, "member_phrase") for member_noun in member_nouns]
            words_to_inflect: Dict[list] = extract_deps(non_incl_sent, member_nouns, annotated=True)

            if words_to_inflect is not None:
              inflection_data.append((member_nouns, words_to_inflect, non_incl_sent_cleaned))

            res = get_res(non_incl_sent_cleaned, member_nouns, inflection_data, allow_return=True)

            # if type(res) == list:
            #   # several sentences generated
            #   for r in res:
            #     sent_id += 1
...
def get_gen_scores(inp_file, out_file=None, write_to_file=False, auto_row=None, print_error_types=False):
  ...
  if write_to_file and out_file is not None:
    ...
    with open(out_file, "w", encoding="utf8") as out:
      writer = csv.writer(out)
      writer.writerow(["id", "auto_incl_sent", "manual_incl_sent", "cos_sim", "wer", "bleu"])

      for i, (auto_incl_sent, manual_incl_sent, cos_sim, wer, bleu) in enumerate(zip(auto_incl_sents, manual_incl_sents, cos_sim_scores, wer_scores, bleu_scores)):
        writer.writerow([i, auto_incl_sent, manual_incl_sent, cos_sim, wer, bleu])
...
def get_deps_score(inp_file, out_file=None):
  ...
  with open(inp_file, "r", encoding="utf8") as inp:
    ...
    for i, row in enumerate(data):
      ...
      rbs_precision, rbs_recall = eval_deps(rbs_deps, manual_deps, _dict=is_dict)
      baseline_precision, baseline_recall = eval_deps(baseline_deps, manual_deps, _dict=is_dict)
  ...
  rbs_count = len(rbs_precisions)
  baseline_count = len(baseline_precisions)
...
def write_deps_to_file(inp_file, out_file, max_sents=250):
  ...
  with open(inp_file, "r", encoding="utf8") as inp:
    ...
    for i, row in enumerate(data):
      ...
      target_nouns = find_targets(nlp(inp_sent))

      rbs_deps_dict = extract_deps(inp_sent, target_nouns)
      baseline_deps_dict = extract_deps_baseline(inp_sent, target_nouns)
